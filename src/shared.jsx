...
// ชื่อโครงการ + รหัสโครงการ (รหัสเดียวกับที่ใช้ออกเลขที่ใบแจ้งซ่อม)
function ProjectLabel({name}){
  const code = window.getProjectCode(name);
  if(!name) return <span style={{color:"var(--muted)"}}>—</span>;
  return <>{code && <span className="mono" style={{color:"var(--muted)",marginRight:6}}>[{code}]</span>}{name}</>;
}
...
// โลโก้บริษัท พานามณี (base64 JPEG) — ใช้ร่วมกันทั้งใบงานและหน้า Login
...
window.buildRepairFormDoc = function(r, user){
  ...
  let rows = "";
  for(let i=0;i<rowCount;i++){ const p=probs[i]; rows += "<tr><td class='c-no'>"+(i+1)+"</td><td class='c-item'>"+fill(p&&p.text)+"</td><td class='c-insp'></td></tr>"; }
...
window.shareRepairImage = async function(r, user){
  ...
  try{
    ...
    // เว้นขอบขาวรอบใบงานก่อนแปลงเป็นรูป (แคปทั้ง body ที่ใส่ padding ไว้ — ไม่กระทบตอนสั่งพิมพ์)
    doc.body.style.padding = "28px 24px";
    doc.body.style.background = "#ffffff";
    const node = doc.body;
    const w = node.scrollWidth||760, h = node.scrollHeight;
...
Object.assign(window, { Badge, CategoryChip, Avatar, ProjectLabel, Modal, Loading, simulate, ProblemsField, getProblems, getRepairPlace });
